...
def _count_nodes(node):
    if 'contents' in node:
        return sum([_count_nodes(child) for child in node['contents']])
    else:
        return 1
...
def _write_node(node, base_url, out_dir, book_tree=False, with_resources=False,
                pbar=None, depth=None, pos={0: 0}, bk_lvl=0):
    print("\n")
    print("%%" * 300)
    print('\n')
    ...
    # Format to write (book tree or flat) as well as a click progress bar, if
    # desired
    print("book_tree: {}".format(book_tree))
    print("with_resources: {}".format(with_resources))
    print("pbar: {}".format(pbar))
    print("depth: {}".format(depth))  # Depth is height of tree
    # Used to reset the lowest bk_lvl counter (pages) per chapter, i.e.
    # pos: {book: ##, chapter: ##, page: ##}
    print("pos: {}".format(pos))
    # Book Levels - 0: book, 1: unit , 2: chapter, 3: module/page, .... for A
    # & P book col11496
    print("bk_lvl: {}".format(bk_lvl))
    ...
    if book_tree:
        out_dir = _make_human_readable_directories(
            out_dir, bk_lvl, pos, node)
    ...
    if 'contents' in node:  # Top-bk_lvl or subcollection - recurse
        bk_lvl += 1
        if bk_lvl not in pos:
            pos[bk_lvl] = 0
        if bk_lvl == depth:  # Reset counter for bottom-most bk_lvl: pages
            pos[bk_lvl] = 0
        for child in node['contents']:
            #  HACK - Silly don't number Preface/Introduction logic
            preface = (
                bk_lvl == 1 and pos[1] == 0 and 'Preface' in child['title'])
            introduction = (
                pos[bk_lvl] == 0 and child['title'] == 'Introduction')
